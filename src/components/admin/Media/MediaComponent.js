...
class MediaComponent extends Component {
    ...
    columns = [
        ...
        },
        {
            ...
            selector: (data, _b) => (
                <>
                    <Button type="button" className="btn btn-success white mr-10" onClick={() => this.onEdit(data.id)}>
                        Cập nhật
                    </Button>
                </>
            ),
    ...
    showMedia(media) {
        ...
        if (media.length > 0) {
            result = media.map((item, index) => {
                return <ItemMedia key={index} index={index} id={item.id} value={item.value} onDelete={this.onDelete} onEdit={this.onEdit} />;
            });
        }
        return result;
    }
    ...
    render() {
        ...
        return (
          ...
          <tbody> {this.showMedia(media)} </tbody>
                ...
                <DataTable title="Youtube" columns={this.columns} data={media} defaultSortField="title" pagination responsive={true} />
                <Modal
                    show={showModal}
                    onHide={() => {
                        this.setState({
