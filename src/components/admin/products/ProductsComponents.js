...
class ProductsComponents extends Component {
    ...
    onPutImg = (id) => {
        const { name, unit, sellPrice, media, selectCategory, description, displayPrice, stockQuantity, isActive, isFeature, shopeeUrl } = this.state;
        var bodyFormData = new FormData();
        for (const file of media) {
            bodyFormData.append('medias', file);
        }
    ...
    onSave = (e) => {
        const { name, id, unit, selectCategory, sellPrice, stockQuantity } = this.state;
        this.setState({
            id: '',
            name: '',
            unit: '',
        ...
        if (id) {
            ...
            if (name === '' || unit === '' || selectCategory === '' || sellPrice === '' || stockQuantity === '') {
    ...
    render() {
        ...
        const result = loading ? <Spinner size={32} speed={1} animating={true} /> : this.showProduct(products);
        return (
            <>
